...
import { UserPlus, User, Shield, Search, Trash2, Users, Activity } from "lucide-react"
...
export default function UsersManagementPage() {
  ...
  useEffect(() => {
    if (!loading && headerRef.current && searchRef.current && usersRef.current && statsRef.current) {
  ...
  const pendingUsers = users.filter(user => user.clerkUserId?.startsWith('temp_')).length
  ...
  return (
    <div className="space-y-6">
      <div ref={headerRef} className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowCreateModal(true)} className="hover:scale-105 transition-transform">
      ...
      <div ref={searchRef} className="flex items-center gap-2">
        ...
        {pendingUsers > 0 && (
          <Badge variant="secondary" className="bg-orange-100 text-orange-700 transition-all hover:scale-105">
